...
define('DMSGUESTBOOKVERSION', "1.4.0");
		...
		<td>
	 	 <table style="width:100%;">
			...
			<!-- guestbook form position -->
     		...
     		<tr><td <? echo $tbc1; ?>>Date / Time format:</td>
...
<?
	...
	"0","1","1","000000","%a, %e %B %Y %H:%M:%S %z","en_EN","top","",
	"0","name@example.com"
	);
...
//--------------------- end of manage guestbook entries ---------------------------
	...
	function initialize_option() {
	  ...
	  if(!get_option("DMSGuestbook_formpos"))				{update_option("DMSGuestbook_formpos",		$option[22]);}
	  if(!get_option("DMSGuestbook_formposlink"))			{update_option("DMSGuestbook_formposlink",	$option[23]);}
	  if(!get_option("DMSGuestbook_send_mail"))				{update_option("DMSGuestbook_send_mail",	$option[24]);}
	  if(!get_option("DMSGuestbook_mail_adress"))			{update_option("DMSGuestbook_mail_adress",	$option[25]);}
	...
	function default_option() {
	  ...
	  update_option("DMSGuestbook_formpos",			$option[22]);
	  update_option("DMSGuestbook_formposlink",		$option[23]);
	  update_option("DMSGuestbook_send_mail",		$option[24]);
	  update_option("DMSGuestbook_mail_adress",		$option[25]);
