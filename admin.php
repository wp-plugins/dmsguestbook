...
//load_plugin_textdomain('dmsguestbook');
//load_textdomain("dmsguestbook", "de_DE.mo");
load_plugin_textdomain('dmsguestbook','/wp-content/plugins/dmsguestbook/language/dmsguestbook-de_DE.mo');
...
//		$moFile = dirname(__FILE__) . "/language/dmsguestbook.mo";
...
define('DMSGUESTBOOKVERSION', "1.15.6");
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
...
?>
    ...
    <ul>
    <li>1.) <?php _e("Create a page which you want to display the DMSGuestbook.", "dmsguestbook") ?></li>
...
<?php
	...
	if(strlen($_SESSION["missing_options"])>0) {
		$_SESSION[fixed_update] = get_option("DMSGuestbook_options") . $_SESSION["missing_options_fixed_update"];
			echo "<b style='width:100%;color:#cc0000;'>" . __("One or more options are missing.") ."</b><br />";
	...
	update_option("DMSGuestbook_options", $restore);
	message("<b>" . __("Update database...") . "</b>", 300, 800);
	...
	if($_REQUEST[action]=="createoption") {
		initialize_option();
	  	message("<b>" . __("DMSGuestbook options") . "<br /></b><br />" . __("Don't forget to set the page id."),260,800);
		echo "<meta http-equiv='refresh' content='0; URL=$location'>";
	}
...
/* backup */
		...
		<input class='button-secondary action' style='font-weight:bold;' type='submit' value='Restore' onclick=\"return confirm('" . __("Would you really like to restore all data?") . "');\" />
		</form>";
...
while (list($key, $val) = each($options)) {
	...
	if($key == "page_id") {
		...
		$c=0;
		$data ="";
		$data .= "<b style='color:#bb0000;'>You have to delete the assigned guestbooks by clicking on \"Clear all\" before you can set this again. Assigned guestbooks will not be released until you press the \"Save\" button.</b>";
		...
		$data .= "
				...
				var m = document.getElementById('countpageid').value;
  				...
  				";
		$data .= "</script>";
		$tooltip = "How to use:<br />1.) Reset all assigned guestbooks by clicking on \'Clear all\'<br />2.) Select your desired language<br /> 3.) Select your guestbook(s) by clicking \'Set\' in ascending order.<br />4.) Press the \'Save\' button";
