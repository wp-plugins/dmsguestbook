...
define('DMSGUESTBOOKVERSION', "1.2.0");
...
<?
	...
	$query_options = mysql_query("SELECT * FROM $table_option WHERE option_name LIKE 'DMSGuestbook_%'");
	$num_rows_option = mysql_affected_rows();

		//if all DMSGuestbook option aren't exist
		if($num_rows_option!=35)
		{
?>
...
?>			<!-- user can delete all DMSGuestbook options from the wp option page -->
			<b style="color:#00bb00">[Status OK] <?=$num_rows_option;?> DMSGuestbook option enries in <?=$table_option;?>.</b>
			<form name="form0" method="post" action="<?=$location;?>">
			...
			Type "delete" to remove all DMSGuestbook option entries from the <?=$table_option;?> table.
	</td></tr></table>
<?
	}
?>
		...
		<td>
	 	 <table style="width:100%;"></tr>
			...
			'<?=get_option("DMSGuestbook_captcha_color");?>');" />
     		...
     		<!-- name text -->
     		<td <? echo $tbc1; ?>>
     		<input style="width:350px;" name="DMSGuestbook_name"
     		value="<?=get_option("DMSGuestbook_name");?>" type="text" />
     		Name text<br />

     		<!-- email text -->
     		<input style="width:350px;" name="DMSGuestbook_email"
     		value="<?=get_option("DMSGuestbook_email");?>" type="text" />
     		Email text<br />

			<!-- url text -->
     		<input style="width:350px;" name="DMSGuestbook_url"
     		value="<?=get_option("DMSGuestbook_url");?>" type="text" />
     		Url text<br />

     		<!-- message text -->
     		<input style="width:350px;" name="DMSGuestbook_message"
     		...
     		<!-- mandatory text -->
     		<input style="width:350px;" name="DMSGuestbook_require"
     		value="<?=get_option("DMSGuestbook_require");?>" type="text" />
     		Mandatory text<br />

     		<!-- submit text -->
     		<input style="width:350px;" name="DMSGuestbook_submit"
     		value="<?=get_option("DMSGuestbook_submit");?>" type="text" />
     		Submit text<br />

			<!-- name error text -->
			<input style="width:350px;" name="DMSGuestbook_name_error"
			value="<?=get_option("DMSGuestbook_name_error");?>" type="text" />
			Name error text<br />

	 		<!-- email error text -->
	 		<input style="width:350px;" name="DMSGuestbook_email_error"
	 		value="<?=get_option("DMSGuestbook_email_error");?>" type="text" />
	 		Email error text<br />

	 		<!-- url error text -->
	 		<input style="width:350px;" name="DMSGuestbook_url_error"
	 		value="<?=get_option("DMSGuestbook_url_error");?>" type="text" />
	 		Url error text<br />

	 		<!-- message error text -->
	 		<input style="width:350px;" name="DMSGuestbook_message_error"
	 		value="<?=get_option("DMSGuestbook_message_error");?>" type="text" />
	 		Message error text<br />

	 		<!-- antispam error text -->
	 		<input style="width:350px;" name="DMSGuestbook_antispam_error"
	 		value="<?=get_option("DMSGuestbook_antispam_error");?>" type="text" />
	 		Antispam error text<br />

	 		<!-- success text -->
	 		<input style="width:350px;" name="DMSGuestbook_success"
			...
			<br />
	 		...
	 		<br />
	 		Requirement: FreetType support -> <? if($array["FreeType Support"]==1) {echo "[FreeType support enabled]";};?>
	 		<br /><br />
	 		<hr style="border: 1px solid;"></hr>
...
<?  		$bgcolor="eeeeee";
			for($x=0; $x<$num_rows1; $x++) {
	    		if($bgcolor=="dddddd") {$bgcolor="eeeeee";} else {$bgcolor="dddddd";}
	 			...
	 			// buid the data / time variable
				$sec=date("s", "{$result[$x][date]}");
				$min=date("i", "{$result[$x][date]}");
				$hour=date("H", "{$result[$x][date]}");
				$day=date("d", "{$result[$x][date]}");
				$month=date("m", "{$result[$x][date]}");
				$year=date("Y", "{$result[$x][date]}");
...
?>
	 			...
	 			<td style="font-size:10px; border:1px solid #eeeeee; background-color:#<? echo $bgcolor; ?>"><?=$result[$x][id];?></td>
	 			<td style="border:1px solid #eeeeee; background-color:#<?=$bgcolor;?>">
	 			<input style="font-size:10px; border:1px solid #eeeeee;" type="text" name="gb_name" value="<?=$result[$x][name];?>"></td>
	 			<td style="border:1px solid #eeeeee; background-color:#<?=$bgcolor;?>">
	 			<textarea style="height:80px; width:500px;font-size:10px;" name="gb_message"><?=$result[$x][message];?></textarea></td>
	 			...
	 			type="text" name="gb_ip" value="<?=$result[$x][ip]; ?>" maxlength="15"></td></tr>
	 			<tr><td style="font-size:10px;">Email: </td> <td><input style="font-size:10px;  width:200px;"
	 			type="text" name="gb_email" value="<?=$result[$x][email];?>"></td></tr>
	 			<tr><td style="font-size:10px;">Url: </td> <td><input style="font-size:10px;  width:200px;"
	 			type="text" name="gb_url" value="<?=$result[$x][url];?>"></td></tr>
				</table>

	 			<td style="font-size:10px; border:1px solid #eeeeee; background-color:#<?=$bgcolor;?>">
	 			<form name="edit_form" method="post" action="<?=$location ?>">
	 			<input name="editdata" value="edit" type="hidden" />
	 			<input name="id" value="<?=$result[$x][id];?>" type="hidden" />
