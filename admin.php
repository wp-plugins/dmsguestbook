...
define('DMSGUESTBOOKVERSION', "1.9.1");
...
?>
    ...
    <li>2.) Save the page and set the page id number value in the red "Page ID" field under "Basic settings".</li>
	  ...
	  <tr>
		<td>
	 	 <table style="width:100%;">
	 		...
	 		<td <?php echo $tbc1; ?>>Put the guestbook page id number here</td>
	 		</tr>
...
?>
		   ...
		   <!-- send mail -->
		    ...
		    <!-- guestbook link -->
	 		<tr><td <?php echo $tbc2; ?>>Guestbook link / Overrule link:</td>
     		<td <?php echo $tbc2; ?>>Guestbook link:<br /><?php echo get_permalink($options[1]); ?><br /><br />
     		Overrule link:<br />
     		<input style="width:400px;" type="text" name="<?php echo $options_name[31][0]; ?>" value="<?php echo $options[31]; ?>">
			</td>
	 		<td <?php echo $tbc2;?>>
	 		You can overrule this link if you have trouble with the guestbook form submit.<br /><br />
	 		Examples:<br />
	 		<?php echo bloginfo('wpurl'); ?>/?p=<?php echo $options[1]; ?><br />
	 		<?php echo bloginfo('wpurl'); ?>/<?php echo $options[1]; ?><br />
	 		<?php echo bloginfo('wpurl'); ?>/YourGuestBookName
	 		</td>
		   </tr>
...
if($_REQUEST[file]!="") {
	...
	/* check the file variable for stylesheet.php file */
	if($_REQUEST[file]=="stylesheet.php") {
	$file=$_REQUEST[file];

	if(file_exists($abspath . "wp-content/plugins/dmsguestbook/" . $file)) {
	$folder="";
	$valid_file=1;
	$save_advanced_button = check_writable($folder, $file);
	} else
...
<?php
	...
	/* check the file variable xxxx.txt & stylesheet.php */
	if(preg_match('/^[a-z0-9]+\.+(txt)/i', $POSTVARIABLE['file'])==1 || $POSTVARIABLE['file']=="stylesheet.php") {
...
/* end of manage guestbook entries */
	...
	function default_options_array() {
		...
		array("url_overruled", "0")						/* You can overrule the url if you have trouble with the guestbook form submit */
		);
	return $options;
	}
	...
	$stringtext = get_option('DMSGuestbook_options');
			...
			$part2 = explode("</" . $options[$c][0] . ">", $part1[1]);
				if($part2[0]=="") {
				$missing_entries .= "&lt;" . $options[$c][0] . "&gt;" . $options[$c][1] . "&lt;/" . $options[$c][0] . "&gt;" . "<br />"; $p++;
				}
			$options[$c] = html_entity_decode($part2[0], ENT_QUOTES);
			/* cut invalid char XSS prevent */
				/* url overruled, need / */
				if($c==31) {
				$options[$c] = preg_replace("/[^a-z-0-9-_,.:?&%=\/]+/i", "", $options[$c]);
				} else {
					   $options[$c] = preg_replace("/[\"\'\`\/\\\\]/i", "", $options[$c]);
					   }
			}

			if($missing_entries!=count($options)) {
			$options[9999]=$missing_entries;
			}
		return $options;
	}
	...
	/* missing options */
	function missing_options($options) {
	global $wpdb;
   	$table_name = $wpdb->prefix . "dmsguestbook";
		echo "<br /><br />
