...
while (list($key, $val) = each($options)) {
	...
	if($key == "page_id") {
		...
		$char_lenght = "";
		$additional = "<br /><span style='font-size:9px;'>(<b style='color:#ee1100;'>NEW!</b> Multiple Guestbooks)</span>";
		$style = "width:150px;background-color:#cc1100;";
		$tooltip = "<b>General:</b><br />Goto \'Manage -> Pages\' and hover with the mouse over your guestbook page.<br />The ID will be shown on the bottom of page<br /><br /><b>Multiple Guestbooks:</b><br />Insert all guestbook id\'s, seperated with commas.<br />The first id is the guestbook 1, the second id is the guestbook 2... et cetera<br />E.g: 3,5,40,20<br /><br /><img src=\'../wp-content/plugins/dmsguestbook/img/wordpress_page_id.png\' /><br /><br />";
		...
		$search_param ="WHERE name LIKE '$_REQUEST[search]' OR email LIKE '$_REQUEST[search]' OR url LIKE '$_REQUEST[search]' OR ip LIKE '$_REQUEST[search]' OR message LIKE '$_REQUEST[search]'";
		...
		else
			{
				...
				$search_param="";
						...
						$search_param="WHERE guestbook = '" . sprintf("%d", $_REQUEST[guestbook]) . "' ";
						}
			}
...
?>
		...
		<?php
			...
			echo "<tr><td><a href='$location?page=Manage&guestbook=all'><b>Guestbook: All</b></a></td></tr>";
			$guestbook_count=1;
			for($m=0; $m<count($multi_page_id); $m++) {
			echo "<tr><td><a href='$location?page=Manage&guestbook=$m'><b>Guestbook $guestbook_count</b></a> (Page id: $multi_page_id[$m])</td></tr>";
			$guestbook_count++;
			}
		?>
		</table>
...
<?php
		...
		$y++;
			...
			echo "<a style='color:#bb1100; text-decoration:none;' href='admin.php?page=Manage&from=$q&select=$y&guestbook=$_REQUEST[guestbook]&search=$_REQUEST[search]'> $y</a>";
			}
			else {
				 echo "<a style='color:#000000; text-decoration:none;' href='admin.php?page=Manage&from=$q&select=$y&guestbook=$_REQUEST[guestbook]&search=$_REQUEST[search]'> $y</a>";
				 }
		}
		echo "</div>
		<br /><br />";
...
?>
		<table class="widefat" style="border:1px solid #aaaaaa; width:100%;">
		<thead>
		<tr <?php echo $tbc3; ?>>
			<th style="padding:0px 5px 0px 5px; width:20px;">ID</th>
	 		<th style="padding:0px 5px 0px 5px; width:550px;">Name / Guestbook assign / Message / Admin review</th>
	 		<th style="padding:0px 5px 0px 5px; width:300px;">Header</th>
...
<?php
			...
			foreach ($query1 as $result) {
				...
				if($result->flag==1) {$bgcolor="F0E9E9";}
...
?>
	 			...
	 			<td style="font-size:10px; border:1px solid #eeeeee; background-color:#<?php echo $bgcolor; ?>"><b><?php echo $result->id;?></b></td>

	 			<td style="border:1px solid #eeeeee; background-color:#<?php echo $bgcolor;?>">
	 			<br />
	 			<b>Name: </b><input style="font-size:10px;" type="text" name="gb_name" value="<?php echo $gbname;?>" />
	 			<b>Guestbook: </b><select name="gb_guestbook">
	 			<?php
	 			...
	 			<textarea style="height:120px; width:550px;font-size:10px;" name="gb_message"><?php echo $gbmsg;?></textarea>
	 			<br />
	 			<?php if($result->flag==1) {$check = "checked"; } else {$check="";} ?>
     			<b>* Admin review:</b> <input type="checkbox" name="gb_flag" value="1" <?php echo $check; ?> />
	 			</td>
	 			...
	 			<td style="text-align:center;font-size:10px; border:1px solid #eeeeee; background-color:#<?php echo $bgcolor;?>">
	 			<form name="edit_form" method="post" action="<?php echo $location ?>">
...
<?php
	...
	if ('edit' == $POSTVARIABLE['editdata']) {
		...
		//if($_REQUEST[gb_guestbook]==0) {$_REQUEST[gb_guestbook];} else {$_REQUEST[gb_guestbook]-1;}
   	...
   	function update_db_fields() {
   		...
   		$table_name = $wpdb->prefix . "dmsguestbook";
      		...
      		dbDelta($sql);
   			...
   			/* add field for guestbooks (> 1.13.0) */
