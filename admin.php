...
define('ROLE', "1");
define('DMSGUESTBOOKVERSION', "1.15.0");
...
while (list($key, $val) = each($options)) {
	...
	if($key == "messagetext_length") {
		...
		$jscript = "";
		$return_messagetext_length = OneInput($key, $label, $type, $entries, $value, $char_lenght, $additional, $style, $tooltip, $jscript);
	}
	...
	if($key == "recaptcha_publickey") {
		...
		$jscript = "";
		$return_recaptcha_publickey = OneInput($key, $label, $type, $entries, $value, $char_lenght, $additional, $style, $tooltip, $jscript);
	}
	...
	if($key == "recaptcha_privatekey") {
		...
		$jscript = "";
		$return_recaptcha_privatekey = OneInput($key, $label, $type, $entries, $value, $char_lenght, $additional, $style, $tooltip, $jscript);
	}
				...
				elseif($key=="css_customize") {
					...
					$save_to_db.="<" . $key . ">" . htmlentities($css_customize, ENT_QUOTES) . "</" . $key . ">\r\n";
						...
						if($key == "page_id") {
							...
							$multi_lang = array_unique($multi_lang);
