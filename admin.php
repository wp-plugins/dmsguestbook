...
define('DMSGUESTBOOKVERSION', "1.14.0");
define('ROLE', "1");
	...
	function add_dmsguestbook() {
		...
		add_menu_page(__('Options', 'dmsguestbook'), __('<span style=\'font-size:12px;\'>DMSGuestbook</span>', 'dmsguestbook'),
		'edit_others_posts', 	'dmsguestbook', 'dmsguestbook_meta_description_option_page', '../wp-content/plugins/dmsguestbook/img/guestbook.png');

		if(current_user_can("level_" . $Role2) || ROLE == 0) {
		add_submenu_page( 'dmsguestbook' , __('Entries', 'dmsguestbook'), __('Entries', 'dmsguestbook'), 'edit_others_posts',
		'Entries', 'dmsguestbook2_meta_description_option_page');
		}

		if(current_user_can("level_" . $Role3) || ROLE == 0) {
		add_submenu_page( 'dmsguestbook' , __('Spam', 'dmsguestbook'), __('Spam', 'dmsguestbook'), 'edit_others_posts',
			...
			if (empty($items)) {
				echo "<li>No items</li>";
			     ...
			     foreach ( $items as $item ) :
					echo "<li><a href='$item[link]' title='$item[title]' target='_blank'>$item[title]</a>&nbsp;&nbsp;<span style='color:#666666;font-size:10px;'>". substr($item[pubdate],5 ,12) . "</span><br />
					" . substr($item[description], 0, 80) . " [...]</li>";
			     endforeach;
			     }
			echo "</ul>
...
while (list($key, $val) = each($options)) {
	...
	if($key == "page_id") {
		...
		foreach ($query_posts as $result) {
			...
			$data .= "<td style='font-size:9px;background-color:#dddddd;padding:2px;'><a href='page.php?action=edit&post=$result->ID'>$result->post_status</a></td>";
				...
				$data .= "<td style='font-size:9px;background-color:#dddddd;padding:2px;'><input class='button-secondary action' style='width:70px;' id='pageid$c' name='pageid$c' type='action' value='$set' $disabled onclick=\"PageID('$result->ID', '$c')\"> </td></tr>";
			$c++;
		}
		$data .= "</table>";
	...
	if($key == "width1") {
		...
		$jscript = "";
		$return_width1 = OneInput($key, $label, $type, $entries, $value, $char_lenght, $additional, $style, $tooltip, $jscript);
	}
	...
	if($key == "width2") {
		...
		$jscript = "";
		$return_width2 = OneInput($key, $label, $type, $entries, $value, $char_lenght, $additional, $style, $tooltip, $jscript);
	}
	...
	if($key == "position1") {
		...
		$jscript = "";
		$return_position1 = OneInput($key, $label, $type, $entries, $value, $char_lenght, $additional, $style, $tooltip, $jscript);
	}
	...
	if($key == "position2") {
		...
		$jscript = "";
		$return_position2 = OneInput($key, $label, $type, $entries, $value, $char_lenght, $additional, $style, $tooltip, $jscript);
	}
	...
	if($key == "navigationsize") {
		...
		$jscript = "";
		$return_navigationsize = OneInput($key, $label, $type, $entries, $value, $char_lenght, $additional, $style, $tooltip, $jscript);
	}
	...
	if($key == "language") {
		...
		$jscript = "";
  		$return_language = SelectBox($key, $label, $option, $value, $additional, $style, $tooltip, $jscript, $jscript);
	}
	...
	if($key == "formposlink") {
		...
		$jscript = "";
		$return_formposlink = OneInput($key, $label, $type, $entries, $value, $char_lenght, $additional, $style, $tooltip, $jscript);
	}
	...
	if($key == "additional_option_title") {
		...
		$jscript = "";
		$return_additional_option_title = OneInput($key, $label, $type, $entries, $value, $char_lenght, $additional, $style, $tooltip, $jscript);
	}
	...
	if($key == "dateformat") {
		...
		$jscript = "";
		$return_dateformat = OneInput($key, $label, $type, $entries, $value, $char_lenght, $additional, $style, $tooltip, $jscript);
	}
	...
	if($key == "setlocale") {
		...
		$jscript = "";
		$return_setlocale = OneInput($key, $label, $type, $entries, $value, $char_lenght, $additional, $style, $tooltip, $jscript);
	}
	...
	if($key == "mail_adress") {
		...
		$additional = "";
		$style = "width:150px;";
		$tooltip = "The email address which the message to be sent is";
		$jscript = "";
		$return_mail_adress = OneInput($key, $label, $type, $entries, $value, $char_lenght, $additional, $style, $tooltip, $jscript);
	}
	...
	if($key == "smtp_host") {
		...
		$jscript = "";
		$return_smtp_host = OneInput($key, $label, $type, $entries, $value, $char_lenght, $additional, $style, $tooltip, $jscript);
	}
	...
	if($key == "smtp_username") {
		...
		$jscript = "";
		$return_smtp_username = OneInput($key, $label, $type, $entries, $value, $char_lenght, $additional, $style, $tooltip, $jscript);
	}
	...
	if($key == "smtp_password") {
		...
		$jscript = "";
		$return_smtp_password = OneInput($key, $label, $type, $entries, $value, $char_lenght, $additional, $style, $tooltip, $jscript);
	}
	...
	if($key == "akismet") {
		...
		$jscript = "onChange=\"akismetContainer();\"";
	...
	if($key == "require_antispam") {
		...
		$additional = "";
		$style = "";
		$tooltip = "Image:<br /><img src=\'../wp-content/plugins/dmsguestbook/captcha/captcha.php\' /><br />If you don\'t see the image here, check the xfiles.ttf and captcha.png permission in your captcha folder<br /><br />Mathematic figures:<br />4 + 9 = 13";
		$jscript = "";
	...
	if($key == "mandatory_char") {
		...
		$jscript = "";
		$return_mandatory_char = OneInput($key, $label, $type, $entries, $value, $char_lenght, $additional, $style, $tooltip, $jscript);
	}
	...
	if($key == "email_image_path") {
		...
		$jscript = "";
		$return_email_image_path = OneInput($key, $label, $type, $entries, $value, $char_lenght, $additional, $style, $tooltip, $jscript);
	}
	...
	if($key == "website_image_path") {
		...
		$jscript = "";
		$return_website_image_path = OneInput($key, $label, $type, $entries, $value, $char_lenght, $additional, $style, $tooltip, $jscript);
	}
	...
	if($key == "url_overruled") {
		...
		$jscript = "";
		$return_url_overruled = OneInput($key, $label, $type, $entries, $value, $char_lenght, $additional, $style, $tooltip, $jscript);
	}
	...
	if($key == "gravatar_size") {
		...
		$jscript = "";
		$return_gravatar_size = OneInput($key, $label, $type, $entries, $value, $char_lenght, $additional, $style, $tooltip, $jscript);
	}
...
echo "<div id='outer'>
	...
	echo "<div class='dbx-box'>
			...
			echo "<div class='dbx-box'>
				...
				<li>&nbsp;</li>
				<div style='display:none;' id='smtp_container'>
...
<?php
			...
			echo "<div class='dbx-box'>
				...
				<div style='display:none;' id='akismet_container'>
...
return $options;
	...
	function OneInput($key, $label, $type, $entries, $value, $char_lenght, $additional, $style, $tooltip, $jscript) {
		$part1 = explode("@", $label);
		...
		unset($data);
			...
			<td>$part1[$x]</td>
			<td><input style='$style;' type='$type' name='$key' id='$key' value='$value' maxlength='$char_lenght' $jscript />$part2[$x]</td><td style='text-align:right;'>$showtooltip</td></tr></table>";
			}
		return $data;
	}
