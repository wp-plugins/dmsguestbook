...
$gb_admin_review		= $options[30];
$gb_url_overrule		= $options[31];
...
$gb_admin_review_text	=	html_entity_decode($language[13], ENT_QUOTES);
...
echo "<div style='$guestbook_font_color'>"; }
		...
		if($_REQUEST[newentry]==1)
		{
			...
			if($gb_require_antispam==1) {
				...
				}else { $antispam_result=0; $error5 =  "$gb_antispam_error";}
			}
	...
	else {if($gb_formpos=="bottom") {echo "<meta http-equiv='refresh' content='0; URL=#guestbookform'>";} }
	...
	if($_REQUEST[widget_gb_step]==1) {$gb_step=1; $num_rows1=1;}

	# read the guestbook
	# if flag = 1 the admin will review this post
	$query2 = $wpdb->get_results("SELECT * FROM $table_name
	WHERE flag != '1' ORDER BY id
	" . sprintf("%s", $gb_sortitem) . " LIMIT " . $from .
	"," . sprintf("%d", $gb_step) . ";");
...
<?php
	...
	$forward ="<a style='$navigation_char' href='$url/index.php?page_id=$gb_page_id&amp;from=$next&amp;select=$_REQUEST[select_forward]'>$gb_forwardarrowchar</a>";
	}

	// navigation char backward construct
	if($back<=-1) {} else {
	$_REQUEST[select_backward]=$select-1;
	$backward = "<a style='$navigation_char' href='$url/index.php?page_id=$gb_page_id&amp;from=$back&amp;select=$_REQUEST[select_backward]'>$gb_backwardarrowchar</a>";
	}
	...
	echo "<div style='$guestbook_message_body'>" . str_replace("\\","",$message_text) . "</div>";
	echo "</div>";
...
?>
	...
	<?php #message ?>
	<div style="<?php echo $textfieldspace;?>">
	<textarea style="<?php echo $messagefield;?>" name="gbmsg" rows="0" cols="0"><?php echo str_replace("\\","",$_REQUEST[gbmsg]);?></textarea>&nbsp;<?php echo $gb_message;?> *</div>
