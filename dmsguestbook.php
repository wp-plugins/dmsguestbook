...
echo "<div style='$guestbook_font_color'>"; }
			...
			if($antispam_result==1) {
				...
				$messagecheck="1"; }
					...
					if($namecheck=='1' && $emailcheck=='1' && $urlcheck=='1' && $messagecheck=='1')
					{
						...
						$sql=$wpdb->query("INSERT INTO $table_name (name, email, url, date, ip, message, flag)
						VALUES ('$nname', '$_REQUEST[gbemail]', '$newurl', '$date', '$ip', '$mmu', '$gb_admin_review')");
	...
	$query1 = $wpdb->get_results("SELECT * FROM  $table_name WHERE flag != '1'");
	$num_rows1 = mysql_affected_rows();

	# read the guestbook
	# if flag = 1 the admin will review this post
	$query2 = $wpdb->get_results("SELECT * FROM $table_name WHERE flag != '1' ORDER BY id $gb_sortitem LIMIT $_REQUEST[from],$gb_step;");
